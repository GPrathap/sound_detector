...
def reconstructFeatureMatrixRealTime(datasetXForConvolution,datasetYForConvolution):
    newXDataSetX = []
    newYDataSetY = []
...
# datasetXLengthForConvolution =len(datasetXForConvolution_test[0])
# datasetYLengthForConvolution = len(datasetYForConvolution_test[0])

a = np.random.rand(1,441000)[0][0:40960]
audoData = api.ClipRealTime(a)
clips_10_test , datasetXForConvolution1, datasetYForConvolution1 , datasetXForFull1, datasetYForFull1 = api.load_dataset_realtime(audoData)
datasetXForConvolution_test,datasetYForConvolution_test = reconstructFeatureMatrixRealTime(datasetXForConvolution1, datasetYForConvolution1)
datasetXLengthForConvolution =len(datasetXForConvolution_test[0])
datasetYLengthForConvolution = len(datasetYForConvolution_test[0])
...
add_layer(h_fc1, 20, datasetYLengthForConvolution, 4)
...
###########################################################
new_saver = tf.train.import_meta_graph('/home/geesara/Documents/KVA/sound_detector/neural_net/my-model-0.meta')
new_saver.restore(sess, '/home/geesara/Documents/KVA/sound_detector/neural_net/my-model-0')
train_op = tf.get_collection('train_op')[0]


# for i in range(2):
#     #saver.restore(sess, "neural_net/neural_net.ckpt")
#     sess.run(train_op, feed_dict={xs: datasetXForConvolution, ys: datasetYForConvolution, keep_prob: 0.5})
#     if i % 2 == 0:
#         test_result = sess.run(train_op, feed_dict={xs: datasetXForConvolution, ys: datasetYForConvolution, keep_prob: 1})
        #test_result = sess.run(merged, feed_dict={xs: datasetXForConvolution_test, ys: datasetYForConvolution_test, keep_prob: 1})
        #print(sess.run(ys, feed_dict={xs: datasetXForConvolution}))
...
prediction1 = tf.get_collection('prediction')[0]
gh = []
gh.append(datasetXForConvolution_test[0])
y_pre1 = sess.run(prediction1, feed_dict={xs: gh, keep_prob: 1})
...
# ckpt = tf.train.get_checkpoint_state(modlePath)
